...
struct node {
    ...
    /* this node's parent */
    Node_T oNParent;
    /* the object containing links to this node's children (must be 
    empty if the node is a file) */
    DynArray_T oDChildren;
    /* TRUE if the node represents a file, FALSE if node represents
    a directory */
    boolean bIsFile; 
    /* pointer to contents of a file node */
    void *pvContents;
...
void *Node_replaceFileContents(Node_T oNNode, void *pvNewContents, 
    size_t ulNewLength) {
        void *pvOldContents = oNNode->pvContents;
